...
router.get(
  ...
  asyncHandler(async (req, res) => {
    if (!idParam.safeParse(req.params.id).success) {
      res.json({ reviews: [], total: 0, page: 1, limit: 20, hasMore: false });
      return;
    }
    const { page, limit } = pageQuery.parse(req.query);
    const data = await reviewsService.listSellerReviews(req.params.id, page, limit);
    const total = data.total;
    const hasMore = page * limit < total;
    res.json({ reviews: data.reviews, total, page, limit, hasMore });
  }),
);
