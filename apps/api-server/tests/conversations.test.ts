...
describe("Conversations API", () => {
  ...
  it("GET /messages supports limit + before pagination", async () => {
    const buyer = await makeUser("Buyer");
    const seller = await makeUser("Seller");
    const buyerSid = await makeSession(buyer.id);
    const sellerSid = await makeSession(seller.id);
    ...
    const conv = await request(app)
      .post("/api/conversations")
      .set("Authorization", bearer(buyerSid))
      .send({ sellerId: seller.id, initialMessage: "m1" });
    ...
    const recentTwo = await request(app)
      .get(`/api/conversations/${conv.body.id}/messages?limit=2`)
      .set("Authorization", bearer(buyerSid));
    expect(recentTwo.body).toHaveLength(2);
    const olderId = recentTwo.body[0].id;

    const olderSlice = await request(app)
      .get(`/api/conversations/${conv.body.id}/messages?limit=10&before=${olderId}`)
      .set("Authorization", bearer(buyerSid));
    expect(olderSlice.body.some((x: { text: string }) => x.text === "m1")).toBe(true);
  });
});
